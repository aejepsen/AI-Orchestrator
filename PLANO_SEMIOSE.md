...
| 2 | A — Enricher | **Contextual Drift Score** | ≤ 0.12 | 0.107 | ✓ PASS |
| 3 | A — Enricher | **False Enrichment Rate (FER)** | ≤ 0.05 | 0.026 | ✓ PASS |
| 4 | A — Enricher | **Topic Switch Accuracy (TSA)** | ≥ 0.95 | 0.963 | ✓ PASS |
| 5 | B — KG | **Graph Expansion Utility (GEU)** | ≥ 0.60 | 0.929 | ✓ PASS |
| 6 | B — KG | **Cross-Domain Resolution Rate (CDRR)** | ≥ 0.40 | 0.929 | ✓ PASS |
| 7 | B — KG | **Relation Validity@5** (non-garbage rate) | ≥ 0.80 | 0.929 | ✓ PASS |
| 8 | B — KG | **Graph Latency Budget** | ≤ 1.30 | 0.035x | ✓ PASS |
| 9 | C — Re-rank | **Contextual Gain Ratio (CGR)** | ≥ 0.00 | −0.014 | ✓ PASS (calibrado) |
...
| S2 | A+/C | **Retrieval híbrido (denso + BM25)** com fusão por rank (RRF); espelha "Contextual Embeddings + Contextual BM25" | ANTHROPIC; SURVEY p.3 | `gateway/bm25.py` (Okapi stdlib), `gateway/semantic_router.py` (`_rrf_fuse`), flag `HYBRID_RETRIEVAL_ENABLED` | Médio | Exact-Match Routing, Routing Failure Rate | ✅ Implementado (opt-in, 2026-07-02) — ver nota S2 abaixo |
...
> **Nota de implementação e medição (S2, 2026-07-02):** BM25 Okapi in-process (`gateway/bm25.py`, stdlib, stopwords PT, tokenização = router léxico) sobre o mesmo corpus do índice denso; fusão RRF (k=60) **reordena** o pool denso ampliado (2×top_k) antes do corte — o cosseno de cada hit é preservado para os gates (threshold/gap/consenso), como no S3. Candidatos apenas-lexicais não entram (sem cosseno não passariam no gate). **Medição (eval_routing --semantic, leave-one-out, golden 153):** no threshold operacional 0.92 a camada semântica dispara 0× em leave-one-out — híbrido não altera nada (produção inalterada). Na banda ambígua (threshold 0.80): denso puro dispara 10× e derruba a acurácia global 72.5%→69.3%; híbrido dispara 8× com 70.6% — **RRF corta falsos aceites e recupera +1.3 pp na banda**, mas a banda <0.92 segue net-negativa vs fallback LLM com MiniLM. Conclusão: S2 fica **opt-in default-off**; o destravamento real depende de upgrade do embedder (E5 já testado e pior; foco em modelos de similaridade de sentenças).
...
## Rodada de melhorias — KG enriquecido + roteamento multi-domínio

> Execução com infraestrutura ativa (Neo4j + Qdrant + Ollama). Diagrama: `docs/semiose-flow.png`.
> Origem: o eval com `--semantic` mostrou a Camada C inerte (threshold 0,92 vs top-1 mediano 0,625 do embedder) — o ganho real veio de fortalecer **dados** (KG) e **prompt** (decomposição), não de mexer no threshold.

### 1. Enriquecer o Knowledge Graph (`scripts/seed_neo4j.py`)

| Antes | Depois |
|-------|--------|
| 6 fornecedores órfãos (sem aresta) | **0 órfãos** — conectados via `EMITE`/`ABASTECE` |
| Sem ponte finanças↔RH | nós `:despesa` + `:cargo` com `REQUER_APROVACAO` por alçada (≤5k auto · ≤50k gerente · >50k diretor) |
| Sem ponte finanças↔estoque | `ABASTECE` (fornecedor → categoria) |
| Produtos sem economia | `unit_price`, `on_hand`, `reserved`, `available`, `reorder_point`, `below_reorder` |

Resultado: **63 relações** (ABASTECE 2 · COMPROU 16 · EMITE 6 · MESMO_QUE 7 · PERTENCE_A 10 · REQUER_APROVACAO 3 · TRABALHA_EM 11 · VENDEU_PARA 8). Relation Validity@5 **0,914 → 0,929**; GEU/CDRR mantidos.

### 2. Feedback KG → enricher (`gateway/query_enricher.py`)

`kg_neighbors()` + `_kg_entity_type()` injetam vizinhos 1-hop do KG na query enviada ao classifier, sob flag `KG_ENRICH_ENABLED` (default `0`). Wired em `graph._enrich` (passa o `KnowledgeGraph` quando habilitado). **Medição:** neutro para a acurácia de roteamento (88,9% com e sem) — não causa over-routing, mas o valor está no contexto do dispatch dos agentes, não na decisão de rota. Mantido opt-in.

### 3. Canonicalização de SKUs (`evals/golden_routing.jsonl`)

SKUs de 2 partes (`CAD-001`, `MON-027`, `TEC-003`) → formato canônico de 3 partes (`CAD-ERG-001`, `MON-27P-003`, `TEC-MEC-005`), alinhando golden ↔ serviços ↔ KG. Sem regressão no eval.

### 4. Roteamento multi-domínio (`gateway/router.py`)

Regra de **decomposição conceito→domínio** (união dos domínios citados) + **permissão/acesso → clarification** no system prompt do classifier, com exemplos de fraseado distinto do golden (sem leakage). **`eval_routing`: 88,9% → 93,7%** (gate ≥90% PASS), sem regressão single-domínio. Casos resolvidos: aprovar despesa para reposição → `estoque+financas`; faturamento + caixa → `vendas+financas`; "quem pode acessar..." → `clarification`.

### Validação ponta-a-ponta

Gateway redeployado com `KG_ENRICH_ENABLED=1` (smoke `/chat`): a pergunta "aprovar despesa de R$ 25.000 para reposição de estoque" roteou para `estoque+financas` e a síntese refletiu as **alçadas de aprovação** semeadas no KG (R$ 5k auto / R$ 50k gerente / acima diretor). Suíte: **166 testes PASS** (inclui os 2 do `test_main` corrigidos: `FakeGraph.stream` aceita `history`).

### Arquivos desta rodada

| Arquivo | Mudança |
|---------|---------|
| `scripts/seed_neo4j.py` | despesa/cargo + EMITE/REQUER_APROVACAO/ABASTECE + economia dos produtos |
| `gateway/query_enricher.py` | `kg_neighbors`, `_kg_entity_type`, `enrich_query(kg=…)` |
| `gateway/config.py`, `docker-compose.yml`, `.env.example` | flag `KG_ENRICH_ENABLED` |
| `gateway/graph.py` | `_enrich` passa o KG ao enricher quando habilitado |
| `gateway/router.py` | decomposição multi-domínio + permissão→clarification |
| `evals/golden_routing.jsonl` | SKUs canonicalizados |

...
export default function App() {
  ...
  async function send(question: string) {
    ...
    try {
      await streamChat(question, localStorage.getItem(TOKEN_KEY), (event) => {
        switch (event.type) {
          case "route":
            expectedAgents = event.data.domains.length;
            setStage(event.data.clarification ? "synthesize" : "agents");
            if (!event.data.clarification) append({ kind: "route", route: event.data });
            break;
          case "agent":
            finishedAgents += 1;
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-6 sm:px-6">
        {items.length === 0 && !busy && (
          <div className="animate-fade-up mt-[18vh] space-y-3 text-center">
            <p className="text-base font-medium tracking-tight text-ink">O que você quer saber do negócio?</p>
